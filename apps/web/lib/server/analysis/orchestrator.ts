...
export class AnalysisOrchestrator {
  ...
  async run(input: AnalysisRunInput): Promise<InvestmentCommitteeState> {
    ...
    try {
      ...
      const bearCase = await executeStage(
        ...
        () => runAgent(
          bearCritic,
          buildBearCriticContext(proposal, fundamentalReports, marketContext, proposalRisk),
          this.options.runner,
        ),
        (output) => output.id,
      );
  ...
  async approve(
    ...
    try {
      ...
      const report = await runAgent(reportWriter, buildReportWriterContext(writerState), this.options.runner);
