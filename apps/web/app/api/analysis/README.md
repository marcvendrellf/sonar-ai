...
- **Offline (`true`, default):** the committee replays the most recent recorded
  run (`fixtures/runs/*.json`, see `lib/server/runs/README.md`) when available;
  otherwise it uses the deterministic golden fixture. Orders apply to the
  internal paper ledger. No keys, no network.
