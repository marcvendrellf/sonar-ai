...
A parallel `feat/saloon-2d-backup` branch now provides a shadcn-only fallback for `/saloon`. It keeps the same deterministic fixture run and inspection flow but replaces WebGL with a responsive committee board built from Cards, Buttons, Avatars, Badges, Progress, Tabs, Tooltips, Scroll Areas, Separators, and Sheets. The branch is a backup if the 3D version fails reliability or presentation-laptop review; the team should select one Saloon implementation rather than merge both compositions.
