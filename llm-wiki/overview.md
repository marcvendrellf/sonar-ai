...
A parallel `feat/saloon-2d-backup` branch now provides a shadcn-only fallback for `/saloon`. Its default view is only a responsive two-by-three grid of large agent cards plus a minimal header and findings button. Tasks, material events, sources, and receipts move into secondary Sheets. The branch maps its visible cast to Market Context, Fundamental Analyst, Portfolio Manager, Bear / Critic, Risk Officer, and Report Writer. It remains a backup if the 3D version fails reliability or presentation-laptop review; the team should select one Saloon implementation rather than merge both compositions.
