...
- **Which local clay-style shell and soft-light configuration meet the Saloon reference?** Closed 2026-08-29: the accepted scene has no room geometry, only an open unsaturated creamy floor, a deep dark-cream table, and matching plinths. The original static set carries base and lightmap UV sets. Blender 5.2.1 LTS and Cycles bake one 3,400 K overhead area key, low warm World fill at strength 0.22, direct light, four-bounce indirect light, and static contacts into a local 2,048 px half-float EXR. Three.js samples it through texture channel 1 on the table and plinths. One warm runtime directional key casts VSM-filtered shadows from every orb and static mesh; restrained hemisphere bounce keeps the sunset treatment readable. The HDR, fake radial shadow cards, emissive orb floor, and accumulated convergence are absent. Only the selected orb floats, hover and selection use damped motion, and no labels or state rings render beneath the orbs. The default right panel contains the agents and their work, with no timeline. Hardware-accelerated Chrome comparison covers the table and interview modes. Presentation-laptop confirmation is still outstanding. See the [asset provenance](../raw-sources/saloon-asset-provenance-2026-08-29.md) and [baked-GI references](../raw-sources/saloon-baked-gi-references-2026-08-29.md).
- **Custom faceless spheres or Live Orb for the Saloon?** Closed 2026-08-29: retain the six custom shared-canvas agent orbs, with smooth floating only for the selected orb. Live Orb remains limited to a single onboarding fallback.
